...
def growth_Atkinson2006(chla, temp, maturity_stage=None, length=None):
    """
    ...
    growth = a + b * length + c * length**2 + (d * chla) / (e + chla) + f * temp + g * temp**2

    return growth


def length_Atkinson2006(chla, temp, initial_length, maturity_stage=None):

    n_days = chla.sizes['days']
    length = xr.zeros_like(chla) * np.nan
    length[0] = initial_length

    for t in range(1, n_days):
        growth = growth_Atkinson2006(
            chla=chla.isel(days=t),
            temp=temp.isel(days=t),
            length=length[t-1],
