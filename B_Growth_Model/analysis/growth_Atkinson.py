...
max_obs_chla = chla_surf_stack.raw_chla.max() 
min_obs_chla = chla_surf_stack.raw_chla.min()
max_obs_temp = temp_100m_stack.avg_temp.max() 
min_obs_temp = temp_100m_stack.avg_temp.min() 

# Print ranges values
print('--- Range of values in Southern Ocean, south of 60°S ---') 
print(f'Max Chla concentration: {max_obs_chla.values:.2f} mg/m3') # 5.00 mg/m3
print(f'Min Chla concentration: {min_obs_chla.values:.2f} mg/m3') # 0.00 mg/m3
print(f'Max temperature: {max_obs_temp.values:.2f} °C') # 7.31 °C
print(f'Max temperature: {min_obs_temp.values:.2f} °C') # -5.03°C
...
if not os.path.exists(file_growth):
    ...
    growth_da = growth_Atkinson2006(chla_surf_stack.raw_chla, temp_100m_stack.avg_temp) #[mm]
