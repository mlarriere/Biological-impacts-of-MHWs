...
#     climatological_length.to_netcdf(os.path.join(path_biomass, f"fake_worlds/clim_length_{stage}.nc"))
#     climatological_mass.to_netcdf(os.path.join(path_biomass, f"fake_worlds/clim_mass_{stage}.nc"))
#     del climatological_length, climatological_mass
#     gc.collect()
...
#     actual_results = [length_single_year(y, stage, chla_surf_SO_allyrs, temp_avg_100m_SO_allyrs) for y in range(39)]
...
#     actual_length.to_netcdf(os.path.join(path_biomass, f"fake_worlds/actual_length_{stage}.nc"))
#     actual_mass.to_netcdf(os.path.join(path_biomass, f"fake_worlds/actual_mass_{stage}.nc"))
#     del actual_length, actual_mass
#     gc.collect()
...
#     noMHWs_results = [length_single_year(y, stage, chla_surf_SO_allyrs, temp_avg_100m_SO_noMHWs) for y in range(39)]
...
#     noMHWs_length.to_netcdf(os.path.join(path_biomass, f"fake_worlds/noMHWs_length_{stage}.nc"))
#     noMHWs_mass.to_netcdf(os.path.join(path_biomass, f"fake_worlds/noMHWs_mass_{stage}.nc"))
#     del noMHWs_length, noMHWs_mass
#     gc.collect()
    
#     # --------- World4. Global Warming
#     print('4. Global Warming')
#     # Run years in parallel
#     # stage='juvenile'
#     # func = partial(length_single_year, stage=stage, chla_ds=chla_surf_SO_allyrs, temp_ds=detrended_temp)
#     # warming_results = process_map(func, range(39), max_workers=10, desc=f"Warming growth | stage={stage}")
#     warming_results = [length_single_year(y, stage, chla_surf_SO_allyrs, detrended_temp) for y in range(39)]

#     warming_length_list, warming_mass_list = zip(*warming_results)
#     warming_length = xr.concat(warming_length_list, dim="years") # shape (39, 181, 231, 1442)
#     warming_mass = xr.concat(warming_mass_list, dim="years") # shape (39, 181, 231, 1442)
    
#     warming_length.to_netcdf(os.path.join(path_biomass, f"fake_worlds/warming_length_{stage}.nc"))
#     warming_mass.to_netcdf(os.path.join(path_biomass, f"fake_worlds/warming_mass_{stage}.nc"))
#     del warming_length, warming_mass
#     gc.collect()
...
def compute_stage(stage):
    ...
    # ---------- World 1: Climatology
    print("  → Climatology")
    clim_length = length_Atkison2006(chla=chla_clim_mean.chla, temp=temp_clim_mean.avg_temp,
...
files = [os.path.join(path_biomass, "fake_worlds/clim_length_stages_SO.nc"),
         ...
         os.path.join(path_biomass, "fake_worlds/warming_length_stages_SO.nc"),
         os.path.join(path_biomass, "fake_worlds/warming_mass_stages_SO.nc"),]
