...
# file_name = 'krillbase_data.csv' 

# krillbase_data = pd.read_csv(os.path.join(path_biomass, file_name), encoding='ISO-8859-1')
# print(krillbase_data.columns)
...
#     'DAYS_FROM_1ST_OCT',
#     'DATE',
#     'TOP_SAMPLING_DEPTH_M',
#     'BOTTOM_SAMPLING_DEPTH_M',
#     'N_OR_S_POLAR_FRONT',
...
# krillbase_selected = krillbase_data[selected_columns]

# # Extract only data south of 60°S
# krillbase_south = krillbase_selected[krillbase_selected['LATITUDE'] < -60].copy()

# # Convert Longitude to 0-360°E
# krillbase_south['LONGITUDE'] = np.where(krillbase_south['LONGITUDE'] < 0, 360 + krillbase_south['LONGITUDE'], krillbase_south['LONGITUDE'])

# # Select only date between 1st november and 30th April
# def is_summer(date):
#     if pd.isnull(date):
#         return False
#     return (date.month >= 11 or date.month <= 4)

# krillbase_south['DATE'] = pd.to_datetime(krillbase_south['DATE'], errors='coerce') #convert to date format
# krillbase_summer = krillbase_south[krillbase_south['DATE'].apply(is_summer)].copy()

# # Select only data in the Atlantic sector (270°E to 360°E)
# krillbase_atlantic = krillbase_summer[(krillbase_summer['LONGITUDE'] >= 270) & (krillbase_summer['LONGITUDE'] <= 360)].copy()

# # Keep only data from 1980 to 2019
# krillbase_atlantic = krillbase_atlantic[(krillbase_atlantic['DATE'] >= '1980-01-01') & (krillbase_atlantic['DATE'] <= '2019-12-31')].copy()

# %% Investigating data
# min_abundance = krillbase_atlantic['NUMBER_OF_KRILL_UNDER_1M2'].min()
# max_abundance = krillbase_atlantic['NUMBER_OF_KRILL_UNDER_1M2'].max()
# mean_abundance = krillbase_atlantic['NUMBER_OF_KRILL_UNDER_1M2'].mean()
# print(f"Minimum krill abundance: {min_abundance}")
# print(f"Mean krill abundance: {mean_abundance}")
# print(f"Maximum krill abundance: {max_abundance}")


# plt.hist(krillbase_atlantic['NUMBER_OF_KRILL_UNDER_1M2'], bins=50, log=True, color='skyblue')
# plt.axvline(mean_abundance, color='red', linestyle='dashed', linewidth=1, label=f'Mean = {mean_abundance:.2f}')
# plt.xlabel('Krill Abundance')
# plt.ylabel('Frequency (log scale)')
# plt.title('Distribution of Krill Abundance')
# plt.legend()
# plt.show()

# print(krillbase_atlantic['NUMBER_OF_KRILL_UNDER_1M2'].sort_values(ascending=False).head())

# %% Select data to plot

# def select_growth_season(data, start_year):
#     start_date = pd.Timestamp(f'{start_year}-11-01')
#     end_date = pd.Timestamp(f'{start_year + 1}-04-30')
#     return data[(data['DATE'] >= start_date) & (data['DATE'] <= end_date)].copy()


# from cartopy.mpl.gridliner import LONGITUDE_FORMATTER, LATITUDE_FORMATTER

# # ---- Define the growth season ----
# start_year = 2015
# data_plot = select_growth_season(krillbase_atlantic, start_year)

# # ---- Output format ----
# plot = 'report'  # or 'slides'
# fig_width, fig_height = (6.3228348611, 5.5) if plot == 'report' else (10, 8)
...
# gridlabel_kwargs = {'size': 9} if plot == 'slides' else {'size': 6}
...
# # ---- Scatter plot ----
# abundance = data_plot['NUMBER_OF_KRILL_UNDER_1M2'].clip(lower=1e-1)  # clip to minimum positive value
# sc = ax.scatter(
#     data_plot['LONGITUDE'],
#     data_plot['LATITUDE'],
#     c=data_plot['NUMBER_OF_KRILL_UNDER_1M2'],
#     cmap='viridis',
#     s=10,
#     transform=ccrs.PlateCarree(),
#     norm=mcolors.LogNorm(vmin=abundance.min(), vmax=abundance.max()),
...
# cbar.set_label('Number of Krill per m² (log scale)', **label_kwargs)

# # ---- Title ----
# season_str = f"{start_year}–{start_year + 1}"
# ax.set_title(f'Krill Observed Abundance (KRILLBASE) \nGrowth Season {season_str}', **title_kwargs)

# plt.tight_layout()
# plt.show()

# # %%
# def select_growth_season_range(data, start_year, end_year):
#     start_date = pd.Timestamp(f'{start_year}-11-01')
#     end_date = pd.Timestamp(f'{end_year}-04-30')
#     return data[(data['DATE'] >= start_date) & (data['DATE'] <= end_date)].copy()

# # Select data from 1980 Nov 1 to 2016 Apr 30
# data_80_16 = select_growth_season_range(krillbase_atlantic, 1980, 2016)

# # Calculate average abundance over the whole period
# avg_abundance = data_80_16['NUMBER_OF_KRILL_UNDER_1M2'].mean()
...
# # Scatter plot
# abundance = data_80_16['NUMBER_OF_KRILL_UNDER_1M2'].clip(lower=1e-1)  # clip to minimum positive value
# sc = ax.scatter(
#     data_80_16['LONGITUDE'],
#     data_80_16['LATITUDE'],
#     c=data_80_16['NUMBER_OF_KRILL_UNDER_1M2'],
#     cmap='viridis',
#     s=10,
#     transform=ccrs.PlateCarree(),
#     norm=mcolors.LogNorm(vmin=abundance.min(), vmax=abundance.max()),
...
# # Colorbar
# cbar = plt.colorbar(sc, ax=ax, orientation='vertical', shrink=0.6, pad=0.05)
# cbar.set_label('Number of Krill per m² (log scale)', fontsize=10)

# # Title and average text
# ax.set_title('Krill Observed Abundance (Nov–Apr, 1980–2016, South of 60°S, Atlantic Sector)', fontsize=11)
# ax.text(
#     0.02, 0.05,
#     f'Average abundance (1980–2016): {avg_abundance:.2f} krill/m²',
